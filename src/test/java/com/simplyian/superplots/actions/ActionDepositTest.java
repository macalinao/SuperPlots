...
public class ActionDepositTest {
    ...
    public void test_perform_notEnoughMoney() {
        ...
        verify(player).sendMessage(contains("don't have that much money"));
    }
    ...
    public void test_perform_success() {
        ...
        when(plot.isOwner("albireox")).thenReturn(true);
        when(player.getName()).thenReturn("albireox");
        when(econ.getBalance("albireox")).thenReturn(400.0);

        List<String> args = Arrays.asList("400");
        action.perform(player, args);

        verify(player).sendMessage(contains("have been deposited"));
        verify(plot).addFunds(400);
        verify(econ).subtractBalance("albireox", 400);
    }
