...
public class ActionShrinkTest {
    private SuperPlotsPlugin main;
    private ActionShrink action;
    ...
    @Test
    public void test_perform_invalidNumber() {
        World world = mock(World.class);
        Location playerLoc = new Location(world, 0, 0, 0);
        when(player.getLocation()).thenReturn(playerLoc);
        Plot plot = mock(Plot.class);
        ...
        List<String> args = Arrays.asList("6a");
        action.perform(player, args);

        verify(player).sendMessage(contains("not a valid"));
    }

    @Test
    public void test_perform_tooSmall() {
        World world = mock(World.class);
        Location playerLoc = new Location(world, 0, 0, 0);
        when(player.getLocation()).thenReturn(playerLoc);
        Plot plot = mock(Plot.class);
        ...
        List<String> args = Arrays.asList("6");
        action.perform(player, args);

        verify(player).sendMessage(contains("minimum size"));
    }
    ...
    public void test_perform_success() {
        ...
        verify(player).sendMessage(contains("has been shrunk"));
        verify(plot).shrink(1);
    }
}
