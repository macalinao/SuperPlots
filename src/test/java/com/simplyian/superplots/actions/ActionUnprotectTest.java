...
public class ActionUnprotectTest {
    private SuperPlots main;
    private ActionUnprotect action;
    ...
    public void setup() {
        ...
        action = new ActionUnprotect(main);
    ...
    @Test
    public void test_perform_alreadyUnprotected() {
        World world = mock(World.class);
        Location playerLoc = new Location(world, 0, 0, 0);
        when(player.getLocation()).thenReturn(playerLoc);
        Plot plot = mock(Plot.class);
        ...
        when(plot.isProtected()).thenReturn(false);
        when(player.getName()).thenReturn("albireox");

        List<String> args = Arrays.asList("asdf");
        action.perform(player, args);

        verify(player).sendMessage(contains("is already unprotected"));
    }
    ...
    public void test_perform_success() {
        ...
        verify(player).sendMessage(contains("has been unprotected"));
        verify(plot).setProtected(false);
    }
